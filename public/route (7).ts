...
const FREE_CURRENCY_REWARDS: GiftDTO[] = [
  { id: -1, slug: "gram-005", name: "0.05 GRAM", rarity: "common", imageUrl: "/images/ton-diamond.png", value: 0.05, chance: 70, rewardType: "currency" },
  { id: -2, slug: "gram-010", name: "0.10 GRAM", rarity: "common", imageUrl: "/images/ton-diamond.png", value: 0.1, chance: 25, rewardType: "currency" },
  { id: -3, slug: "gram-025", name: "0.25 GRAM", rarity: "rare", imageUrl: "/images/ton-diamond.png", value: 0.25, chance: 4.8, rewardType: "currency" },
]
    ...
    return {
      ...
      items: c.isFree
        ...
        .sort((a, b) => b.value - a.value),
          { id: -101, slug: `gram-small-${c.id}`, name: `${Math.max(1, Math.round(Number(c.price) * 0.1))} GRAM`, rarity: "common", imageUrl: "/images/ton-diamond.png", value: Math.max(1, Math.round(Number(c.price) * 0.1)), chance: 20, rewardType: "currency" as const },
          { id: -102, slug: `gram-medium-${c.id}`, name: `${Math.max(1, Math.round(Number(c.price) * 0.25))} GRAM`, rarity: "rare", imageUrl: "/images/ton-diamond.png", value: Math.max(1, Math.round(Number(c.price) * 0.25)), chance: 12, rewardType: "currency" as const },
          { id: -103, slug: `gram-large-${c.id}`, name: `${Math.max(1, Math.round(Number(c.price) * 0.5))} GRAM`, rarity: "epic", imageUrl: "/images/ton-diamond.png", value: Math.max(1, Math.round(Number(c.price) * 0.5)), chance: 6, rewardType: "currency" as const },
          { id: -104, slug: `gram-jackpot-${c.id}`, name: `${Math.max(1, Math.round(Number(c.price)))} GRAM`, rarity: "legendary", imageUrl: "/images/ton-diamond.png", value: Math.max(1, Math.round(Number(c.price))), chance: 2, rewardType: "currency" as const },
        ],
    }
  })
}
  ...
  return db.transaction(async (tx) => {
    ...
    if (caseRow.isFree) {
      ...
      const won: GiftDTO = {
        ...
        imageUrl: "/images/ton-diamond.png",
        value: currencyValue,
        rewardType: "currency",
      }
      await tx.insert(gameHistory).values({
