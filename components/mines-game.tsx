...
import { Bomb, Bone, ChevronUp, Clover, Loader2, PawPrint, RotateCcw, ShieldCheck, Sparkles, Trophy } from "lucide-react"
...
export function MinesGame() {
  ...
  return <main className="relative mx-auto min-h-[calc(100dvh-58px)] w-full max-w-[980px] overflow-hidden bg-[radial-gradient(circle_at_50%_-10%,#1e6540_0%,#10281b_35%,#08130e_78%)] px-3 pb-[calc(7rem+var(--tg-content-safe-area-inset-bottom,0px))] pt-3 text-white md:px-5 md:pt-5">
    ...
    <section className="relative grid gap-3 md:grid-cols-[minmax(0,1fr)_300px]">
      <div className="order-2 overflow-hidden rounded-[30px] border border-white/[.08] bg-[#081a10]/90 p-2.5 shadow-[0_20px_70px_rgba(0,0,0,.3),inset_0_1px_0_rgba(255,255,255,.05)] md:order-1 md:p-4">
        <div className="mb-2.5 grid grid-cols-3 gap-1.5 md:mb-4 md:gap-2"><Metric label="Safe spots" value={`${safeOpened}/${safeTotal}`} icon={PawPrint} tone="emerald" /><Metric label="Current" value={`${(round?.multiplier ?? 1).toFixed(2)}×`} icon={Sparkles} tone="amber" /><Metric label="Next bone" value={`${(round?.nextMultiplier ?? 1).toFixed(2)}×`} icon={ChevronUp} tone="blue" /></div>
        ...
        <div className="rounded-[25px] bg-black/20 p-2 ring-1 ring-white/[.045] md:p-3">
          ...
          return <button key={tile} aria-label={`Tile ${tile + 1}`} onClick={() => reveal(tile)} disabled={!active || opened || busyTile != null} className={cn("group relative aspect-square overflow-hidden rounded-[13px] border border-white/[.055] transition duration-200 sm:rounded-[18px]", opened && !mine ? "translate-y-[2px] bg-[linear-gradient(145deg,#a3e635,#34d399)] text-[#092014] shadow-[inset_0_2px_7px_rgba(255,255,255,.42),0_0_18px_rgba(52,211,153,.2)]" : mine && ended ? "bg-[radial-gradient(circle_at_50%_30%,#fb7185,#881337)] text-white shadow-[0_0_24px_rgba(244,63,94,.35)]" : "bg-[linear-gradient(145deg,#2c6540,#173a25)] text-emerald-100/18 shadow-[0_4px_0_#0b2415,inset_0_1px_0_rgba(255,255,255,.08)] hover:-translate-y-0.5 hover:brightness-110 active:translate-y-1 active:shadow-none", busyTile === tile && "animate-pulse")}>{busyTile === tile ? <Loader2 className="mx-auto h-5 w-5 animate-spin text-emerald-200" /> : mine && ended ? <><img src="/images/puggift-bot-avatar-web-v2.webp" alt="Angry pug" className="absolute inset-1 h-[calc(100%-8px)] w-[calc(100%-8px)] animate-[bounce_.7s_ease-in-out_2] rounded-[11px] object-cover sm:rounded-[15px]" /><Bomb className="absolute bottom-1 right-1 h-3.5 w-3.5 rounded-full bg-rose-950/75 p-0.5" /></> : opened ? <><span className="absolute inset-1 rounded-[10px] border border-white/25" /><PawPrint className="mx-auto h-6 w-6 sm:h-8 sm:w-8" fill="currentColor" /></> : <Bone className="mx-auto h-5 w-5 rotate-[-24deg] transition group-hover:rotate-0 group-hover:scale-110 sm:h-6 sm:w-6" />}</button>
          })}</div>
        </div>
      </div>

      <aside className="order-1 flex flex-col gap-3 rounded-[28px] border border-white/[.08] bg-[#142d1e]/95 p-3.5 shadow-[0_16px_50px_rgba(0,0,0,.25),inset_0_1px_0_rgba(255,255,255,.05)] md:order-2 md:self-start md:p-4">
        {!round ? <><div className="flex items-center justify-between"><div><span className="text-[8px] font-black uppercase tracking-[.16em] text-emerald-300/55">New expedition</span><h2 className="font-display text-lg font-black">Choose your risk</h2></div><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-300/10 text-emerald-300"><ShieldCheck className="h-4 w-4" /></span></div>
          ...
          <div><div className="mb-1.5 flex items-center justify-between"><span className="text-[8px] font-black uppercase tracking-[.14em] text-white/35">Angry pugs</span><span className="text-[9px] font-bold text-white/30">More = bigger wins</span></div><div className="grid grid-cols-4 gap-1.5">{MINE_OPTIONS.map((count) => <button key={count} onClick={() => setMineCount(count)} className={cn("flex min-h-11 flex-col items-center justify-center rounded-xl text-xs font-black transition", mineCount === count ? "bg-gradient-to-b from-lime-300 to-emerald-400 text-emerald-950 shadow-[0_4px_0_#10743d]" : "bg-white/[.06] text-white/42 ring-1 ring-white/[.04]")}><Bomb className="mb-0.5 h-3 w-3" />{count}</button>)}</div></div></> : <div className="rounded-2xl bg-black/20 p-3 ring-1 ring-white/[.055]"><div className="flex items-center justify-between"><div><span className="text-[8px] font-black uppercase tracking-[.16em] text-white/35">Live expedition</span><div className="mt-1 flex items-center gap-2 font-display text-lg font-black"><Coin className="h-5 w-5" />{fmt(round.payout)}</div></div><span className={cn("flex h-11 w-11 items-center justify-center rounded-2xl", active ? "bg-emerald-300/10 text-emerald-300" : round.status === "bust" ? "bg-rose-400/10 text-rose-300" : "bg-amber-300/10 text-amber-300")}>{active ? <Bone className="h-5 w-5" /> : round.status === "bust" ? <Bomb className="h-5 w-5" /> : <Trophy className="h-5 w-5" />}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-[9px] font-bold text-white/38"><span className="rounded-xl bg-white/[.04] px-2 py-2">Pugs <b className="float-right text-white">{round.mineCount}</b></span><span className="rounded-xl bg-white/[.04] px-2 py-2">Bones <b className="float-right text-white">{safeOpened}</b></span></div></div>}

        {error && <p className="rounded-xl bg-rose-500/12 px-3 py-2 text-center text-[10px] font-bold text-rose-200">{error}</p>}
        {!round ? <button onClick={start} disabled={busy || bet < 1 || bet > balance} className="mt-auto flex min-h-13 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-lime-300 to-emerald-400 font-display text-base font-black text-emerald-950 shadow-[0_6px_0_#10743d,0_14px_28px_rgba(16,185,129,.18)] transition active:translate-y-1 active:shadow-none disabled:translate-y-0 disabled:opacity-35">{busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <PawPrint className="h-5 w-5" fill="currentColor" />}Start bone hunt</button> : active ? <button onClick={cashout} disabled={!round.revealed.length || busy} className="mt-auto flex min-h-13 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-amber-300 to-orange-300 font-display text-base font-black text-amber-950 shadow-[0_6px_0_#9a5d10,0_14px_28px_rgba(251,191,36,.15)] transition active:translate-y-1 active:shadow-none disabled:translate-y-0 disabled:opacity-35"><Coin className="h-5 w-5" />Cash out · {fmt(round.payout)}</button> : <div className="mt-auto space-y-2 text-center"><div className={cn("rounded-2xl px-3 py-3 font-display text-base font-black", round.status === "bust" ? "bg-rose-500/15 text-rose-300 ring-1 ring-rose-300/10" : "bg-emerald-400/15 text-emerald-300 ring-1 ring-emerald-300/10")}>{round.status === "bust" ? "The angry pug caught you" : `Treasure secured · ${fmt(round.payout)}`}</div><button onClick={reset} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-white/[.08] text-sm font-black ring-1 ring-white/[.06]"><RotateCcw className="h-4 w-4" />Hunt again</button></div>}
      </aside>
    </section>
  </main>
}

function Metric({ label, value, icon: Icon, tone }: { label: string; value: string; icon: typeof PawPrint; tone: "emerald" | "amber" | "blue" }) {
