...
export function DiceGame() {
  ...
  return <main className="game-surface game-surface--upgrade relative mx-auto min-h-[calc(100dvh-64px)] w-full max-w-[980px] overflow-hidden px-3 pb-[calc(7rem+var(--tg-content-safe-area-inset-bottom,0px))] pt-3 text-white md:px-5 md:pt-5">
    <div className="pointer-events-none absolute inset-0 opacity-20 [background-image:radial-gradient(rgba(255,255,255,.32)_1px,transparent_1px)] [background-size:46px_46px]" />
    <header className="relative overflow-hidden rounded-[30px] bg-[linear-gradient(125deg,#30245c,#251b49_55%,#171426)] p-5 ring-1 ring-white/10 md:p-7"><div className="relative z-10 max-w-[62%]"><span className="text-[9px] font-black uppercase tracking-[.18em] text-fuchsia-200/70">Instant game</span><h1 className="mt-1 font-display text-3xl font-black md:text-4xl">Pug Dice</h1><p className="mt-2 text-[11px] font-bold leading-relaxed text-white/50">Pick the risk. Roll under the target. Win instantly.</p></div><img src="/images/puggift-mark-v3.svg" alt="Pug Dice" className="absolute -bottom-8 right-2 h-36 w-36 rotate-3 rounded-[40px] object-cover opacity-90 drop-shadow-2xl md:right-8 md:h-44 md:w-44" /></header>

    <section className="relative mt-3 grid gap-3 md:grid-cols-[1fr_310px]">
      <div className="app-panel flex min-h-[330px] flex-col items-center justify-center rounded-[30px] p-5">
      ...
      <aside className="app-panel rounded-[30px] p-4 md:p-5"><div className="text-[9px] font-black uppercase tracking-[.16em] text-fuchsia-200/55">Risk multiplier</div><div className="mt-2 grid grid-cols-4 gap-1.5 md:grid-cols-2">{MULTIPLIERS.map((value) => <button key={value} onClick={() => setMultiplier(value)} disabled={rolling} className={cn("rounded-2xl py-3 text-sm font-black transition", value === multiplier ? "bg-[#8b5cf6] text-white shadow-[0_5px_0_#5b32a8]" : "bg-white/[.07] text-white/48")}>{value}×</button>)}</div>
