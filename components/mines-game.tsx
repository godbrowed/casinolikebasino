...
import { Bone, ChevronUp, Clover, Loader2, PawPrint, RotateCcw, ShieldCheck, Sparkles, Trophy } from "lucide-react"
    ...
    <section className="relative grid gap-3 md:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <aside className="order-1 flex flex-col gap-3 rounded-[28px] border border-white/[.08] bg-[#142d1e]/95 p-3.5 shadow-[0_16px_50px_rgba(0,0,0,.25),inset_0_1px_0_rgba(255,255,255,.05)] md:order-2 md:self-start md:p-4">
        {!round ? <><div className="flex items-center justify-between"><div><span className="text-[8px] font-black uppercase tracking-[.16em] text-emerald-300/55">New expedition</span><h2 className="font-display text-lg font-black">Choose your risk</h2></div><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-300/10 text-emerald-300"><ShieldCheck className="h-4 w-4" /></span></div>
          ...
          <div><div className="mb-1.5 flex items-center justify-between"><span className="text-[8px] font-black uppercase tracking-[.14em] text-white/35">Angry pugs</span><span className="text-[9px] font-bold text-white/30">More = bigger wins</span></div><div className="grid grid-cols-4 gap-1.5">{MINE_OPTIONS.map((count) => <button key={count} onClick={() => setMineCount(count)} className={cn("flex min-h-12 items-center justify-center gap-1.5 rounded-xl text-xs font-black transition", mineCount === count ? "bg-gradient-to-b from-lime-300 to-emerald-400 text-emerald-950 shadow-[0_4px_0_#10743d]" : "bg-white/[.06] text-white/42 ring-1 ring-white/[.04]")}><img src="/images/puggift-bot-avatar-web-v2.webp" alt="" className={cn("h-6 w-6 rounded-full object-cover ring-1", mineCount === count ? "ring-emerald-950/25" : "opacity-55 grayscale ring-white/10")} />{count}</button>)}</div></div></> : <div className="rounded-2xl bg-black/20 p-3 ring-1 ring-white/[.055]"><div className="flex items-center justify-between"><div><span className="text-[8px] font-black uppercase tracking-[.16em] text-white/35">Live expedition</span><div className="mt-1 flex items-center gap-2 font-display text-lg font-black"><Coin className="h-5 w-5" />{fmt(round.payout)}</div></div><span className={cn("flex h-11 w-11 items-center justify-center overflow-hidden rounded-2xl", active ? "bg-emerald-300/10 text-emerald-300" : round.status === "bust" ? "bg-rose-400/10 text-rose-300 ring-1 ring-rose-300/20" : "bg-amber-300/10 text-amber-300")}>{active ? <Bone className="h-5 w-5" /> : round.status === "bust" ? <img src="/images/puggift-bot-avatar-web-v2.webp" alt="Angry pug" className="h-full w-full object-cover" /> : <Trophy className="h-5 w-5" />}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-[9px] font-bold text-white/38"><span className="rounded-xl bg-white/[.04] px-2 py-2">Pugs <b className="float-right text-white">{round.mineCount}</b></span><span className="rounded-xl bg-white/[.04] px-2 py-2">Bones <b className="float-right text-white">{safeOpened}</b></span></div></div>}
