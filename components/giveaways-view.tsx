...
function GiveawayCard({ item, pending, onJoin, onDraw }: { item: GiveawayDashboard["giveaways"][number]; pending: boolean; onJoin: (ticketCount: number) => void; onDraw: () => void }) {
  ...
  return <article id={`giveaway-${item.id}`} className="app-panel overflow-hidden rounded-[25px]">
    ...
    <div className={cn("grid gap-px bg-white/[.055]", paid ? "grid-cols-4" : "grid-cols-3")}><CardStat value={String(item.participantCount)} label="players" /><CardStat value={String(item.ticketCount)} label="tickets" /><CardStat value={String(item.winnerCount)} label="winners" />{paid && <CardStat value={fmt(item.pot)} label="bank" coin />}</div>
...
function Creator({ dashboard, pending, error, success, onBack, onRefresh, setError, setSuccess, startTransition }: { dashboard: GiveawayDashboard; pending: boolean; error: string; success: string; onBack: () => void; onRefresh: () => void; setError: (value: string) => void; setSuccess: (value: string) => void; startTransition: (callback: () => Promise<void>) => void }) {
  ...
  const [ticketLimit, setTicketLimit] = useState("100000")
  const [winnerCount, setWinnerCount] = useState("1")
  const activeChannel = dashboard.channels.find((channel) => channel.active && channel.username?.toLowerCase() === channelUsername.replace(/^@+/, "").toLowerCase())
  useEffect(() => {
  ...
  return <div className="mx-auto w-full max-w-[760px] space-y-4">
    ...
    <section className="app-panel rounded-[27px] p-4 sm:p-5">
    ...
    <section className="app-panel rounded-[27px] p-4 sm:p-5">
      ...
      <Field label={`NFT prizes · ${inventoryIds.length} selected`}><div className="grid max-h-64 grid-cols-2 gap-2 overflow-y-auto pr-1 sm:grid-cols-3">{dashboard.availableGifts.map((gift) => { const selected = inventoryIds.includes(gift.inventoryId); return <button type="button" key={gift.inventoryId} onClick={() => setInventoryIds((current) => selected ? current.filter((id) => id !== gift.inventoryId) : [...current, gift.inventoryId])} className={cn("relative flex min-w-0 flex-col items-center rounded-2xl bg-black/20 p-2.5 text-center ring-2 transition", selected ? "ring-[#5c8cff]" : "ring-transparent")}><img src={gift.imageUrl} alt={gift.name} className="h-20 w-20 object-contain" /><b className="mt-1 w-full truncate text-[11px]">{gift.name}</b><span className="mt-1 flex items-center gap-1 text-[9px] text-white/42"><Coin className="h-3 w-3" />{fmt(gift.value)}</span>{selected && <span className="absolute right-2 top-2 flex h-5 w-5 items-center justify-center rounded-full bg-[#3674ff]"><Check className="h-3 w-3" /></span>}</button>})}</div>{dashboard.availableGifts.length === 0 && <div className="rounded-2xl border border-dashed border-white/10 p-5 text-center text-xs text-white/40">No available NFT gifts. Open a case or deposit a gift first.</div>}</Field>
      ...
      <div className="grid grid-cols-2 gap-3">{mode === "paid" && <Field label="Ticket price"><div className="relative"><Coin className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2" /><input inputMode="decimal" className="giveaway-input pl-10" value={price} onChange={(event) => setPrice(event.target.value.replace(/[^0-9.]/g, ""))} /></div></Field>}<Field label="Winners"><input inputMode="numeric" className="giveaway-input" value={winnerCount} onChange={(event) => setWinnerCount(String(Math.max(1, Math.min(inventoryIds.length || 1, Number(event.target.value.replace(/\D/g, "")) || 1))))} /></Field>{mode === "paid" && <Field label="Ticket limit per player"><input inputMode="numeric" className="giveaway-input" value={ticketLimit} onChange={(event) => setTicketLimit(String(Math.max(1, Math.min(100000, Number(event.target.value.replace(/\D/g, "")) || 1))))} /></Field>}<Field label="Duration"><select className="giveaway-input" value={duration} onChange={(event) => setDuration(event.target.value)}><option value="5">5 min</option><option value="60">1 hour</option><option value="360">6 hours</option><option value="1440">24 hours</option><option value="4320">3 days</option><option value="10080">7 days</option><option value="43200">30 days</option></select></Field></div>
      <button disabled={pending || !activeChannel || !inventoryIds.length || !title.trim()} onClick={publish} className="mt-1 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#3674ff] text-sm font-black shadow-[0_8px_0_#193e9d] active:translate-y-1 active:shadow-none disabled:opacity-35">{pending ? <LoaderCircle className="h-5 w-5 animate-spin" /> : <Send className="h-5 w-5" />}Lock NFTs & publish</button>
    </section>
  </div>
}
