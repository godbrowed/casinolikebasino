...
export function LanguageProvider({children}:{children:ReactNode}) { const [language,setLanguageState]=useState<Language>("en"); useEffect(()=>{const x=localStorage.getItem("giftlys-language");if(x==="en"||x==="ru")setLanguageState(x)},[]); const setLanguage=(x:Language)=>{setLanguageState(x);localStorage.setItem("giftlys-language",x);document.documentElement.lang=x}; return <Ctx.Provider value={{language,setLanguage,t:(key)=>strings[language][key]}}>{children}</Ctx.Provider> }
